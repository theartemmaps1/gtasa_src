...
#include "CustomPipelinesCommonXbox.h"
...
//
#define CARFX_D3D8_SPEC_LIGHT_INDEX			(1)

D3DLIGHT8							CCustomCarEnvMapPipeline::m_gSpecularLight;	// global specular light used by the pipe
...
void CCustomCarEnvMapPipeline::PreRenderUpdate()
{
	...
	// update specular light:
	{
		...
		// specular parameters:
		//
	D3DLIGHT8 *pLight = &m_gSpecularLight;
		ZeroMemory(pLight, sizeof(D3DLIGHT8));
...
static RwBool   setMaterial = FALSE;
static RwBool   setMaterialColor = FALSE;
static RwBool   modulateMaterial = FALSE;
static RwInt32  bLightingEnabled = 0;

//
// Set up per-mesh material renderstate when lighting using the fixed function pipeline.
//
//
static 
void Custom_rxXbDefaultRenderFFPMeshSetUp(RxXboxInstanceData *mesh, float matSpecular=0.0f, float matSpecPower=0.0f)
{

static D3DMATERIAL8 D3D8material =
{
...
0.0f /* Power */
	...
	D3D8material.Specular.r = D3D8material.Specular.g = D3D8material.Specular.b = matSpecular;
	D3D8material.Power = matSpecPower;


    if(1)	// (setMaterial)
    {
        const RpMaterial *material = mesh->material;

//        if ( LastColor != *((const RwUInt32 *)&(material->color)) ||
//             FLOATASINT(LastAmbientScale) != FLOATASINT(material->surfaceProps.ambient) ||
//             FLOATASINT(LastDiffuseScale) != FLOATASINT(material->surfaceProps.diffuse) )
        {
			float LastAmbientScale = material->surfaceProps.ambient;
			float LastDiffuseScale = material->surfaceProps.diffuse;

            if (setMaterialColor && *((const RwUInt32 *)&(material->color)) != 0xffffffff)
            {
                const RwRGBA  *color;
                RwReal  ambScale;
                RwReal  diffScale;
                ...
                //LastColor = *((const RwUInt32 *)color);

                ambScale = LastAmbientScale * COLORSCALAR;
                diffScale = LastDiffuseScale * COLORSCALAR;

                D3D8material.Diffuse.r = color->red * diffScale;
                D3D8material.Diffuse.g = color->green * diffScale;
                D3D8material.Diffuse.b = color->blue * diffScale;
                D3D8material.Diffuse.a = color->alpha * COLORSCALAR;

                D3D8material.Ambient.r = color->red * ambScale;
                D3D8material.Ambient.g = color->green * ambScale;
                D3D8material.Ambient.b = color->blue * ambScale;
                D3D8material.Ambient.a = color->alpha * COLORSCALAR;
            }
            else
            {
                //LastColor = 0xffffffff;

                D3D8material.Diffuse.r = LastDiffuseScale;
                D3D8material.Diffuse.g = LastDiffuseScale;
                D3D8material.Diffuse.b = LastDiffuseScale;
                D3D8material.Diffuse.a = 1.f;

                D3D8material.Ambient.r = LastAmbientScale;
                D3D8material.Ambient.g = LastAmbientScale;
                D3D8material.Ambient.b = LastAmbientScale;
                D3D8material.Ambient.a = 1.f;
            }

            D3DDevice_SetMaterial(&D3D8material);
        }
    }
...
//
//
void CCustomCarEnvMapPipeline::CustomPipeRenderCB(RxXboxResEntryHeader *resEntryHeader, void *object, RwUInt8 type, RwUInt32 flags)
{

const rpMatFXMaterialData   *matFXData;
DWORD						effect;		
const float envMapLightMult = 1.0f;
float materialSpecPower, materialSpecular;
	...
	const RwMatrix		*transform	= RwFrameGetLTM(RpAtomicGetFrame(pAtomic));
	const RpXboxVtxFmt	*format		= RpXboxGeometryGetVtxFmt(RpAtomicGetGeometry((RpAtomic *)object));

    // Get lighting state:
    RwXboxGetCachedRenderState(D3DRS_LIGHTING, (DWORD*)&bLightingEnabled);

    // use the fixed function pipeline's lighting:
//	_rxXbDefaultRenderFFPObjectSetUp(resEntryHeader, object, type, flags);
    /*
    Use D3D's materials if we're got lighting and the object is supposed to be lit,
    otherwise use our tfactor version to blend material color with unlit objects
    or force lit objects with no lights to be black.
    */
    if(bLightingEnabled && (flags&rxGEOMETRY_LIGHT))
    {
        setMaterial = TRUE;
        if (flags & rxGEOMETRY_MODULATE)
        {
            if (flags & rxGEOMETRY_PRELIT)
            {
                ...
                modulateMaterial = TRUE;
            }
            else
            {
                setMaterialColor = TRUE;
                modulateMaterial = FALSE;
            }
        }
        else
        {
...
//        LastColor = 0;
//        LastAmbientScale = FLT_MAX;
//        LastDiffuseScale = FLT_MAX;
    }
    else
    {
        setMaterial = FALSE;
    ...
    if(bLightingEnabled)
    {
        if (flags & rxGEOMETRY_PRELIT)
        {
		    ...
		    _rwXbRenderStateForce(D3DRS_COLORVERTEX, TRUE);
            _rwXbRenderStateForce(D3DRS_EMISSIVEMATERIALSOURCE, D3DMCS_COLOR1);
        }
        else
        {
            // Emmisive color from material, set to black in the submit node:
		    _rwXbRenderStateForce(D3DRS_COLORVERTEX, FALSE);
            _rwXbRenderStateForce(D3DRS_EMISSIVEMATERIALSOURCE, D3DMCS_MATERIAL);
        }

        // get the alpha from the vertex colors if there is any:
	    if (resEntryHeader->vertexAlpha)
        {
            _rwXbRenderStateForce(D3DRS_DIFFUSEMATERIALSOURCE, D3DMCS_COLOR1);
        }
        else
        {
            _rwXbRenderStateForce(D3DRS_DIFFUSEMATERIALSOURCE, D3DMCS_MATERIAL);
        }
    }
    ...
    //
    // Set the stream source - all meshes in object are bunged in the vertex buffer
    //
    D3DDevice_SetStreamSource(0,
                              (D3DVertexBuffer *)resEntryHeader->vertexBuffer,
                              resEntryHeader->stride);

    for(RxXboxInstanceData *mesh = resEntryHeader->begin; mesh!=resEntryHeader->end; mesh++)
    {
//////////////////////////////////////////////////////////////////////////////////////////////////

		RpMaterial *pMaterial = mesh->material;
		ASSERT(pMaterial);
...
const bool bUseSpecular		= (matPassMask&CARFX_SPECMAP_PASS_BIT) && (!bNoExtraPasses);




#if 0
		/* Do we have a multitex object in there */
       	RpMultiTexture      *multiTexture = RpMaterialGetMultiTexture(mesh->material, rwID_XBOX);
        if( multiTexture &&
            multiTexture->effect &&
            multiTexture->effect->platformID == rwID_XBOX)
        {
            /* Use the multitexturing */
            rxXbAtomicMatFXMTRender(object, resEntryHeader, mesh, flags, multiTexture);
        }
        else
#endif
		...
		RwBool	alpha=FALSE;
        if(	(resEntryHeader->vertexAlpha)			||
			(mesh->material->color.alpha < 0xFF)	)
        {
            alpha = TRUE;
        }
        else
        ...
        RwXboxSetCachedRenderState(D3DRS_DIFFUSEMATERIALSOURCE, D3DMCS_MATERIAL);
        RwXboxSetCachedRenderState(D3DRS_AMBIENTMATERIALSOURCE, D3DMCS_MATERIAL);
        RwXboxSetCachedRenderState(D3DRS_COLORVERTEX, TRUE);



#if 0
        RwBool	dualPass=FALSE;
/*
		if (mesh->material->texture != NULL)
        {
            RwXboxRasterExtension   *rasExt = RwXboxRasterGetExtension(RwTextureGetRaster(mesh->material->texture));
...
*/
        if ( !dualPass )
        {
                ...
                (flags & rxGEOMETRY_MODULATE))
            {
                ...
                const RwReal MaterialColor[4]=
                {
                    (RwReal)mesh->material->color.red * COLORSCALAR,
                    (RwReal)mesh->material->color.green * COLORSCALAR,
                    (RwReal)mesh->material->color.blue * COLORSCALAR,
                    (RwReal)mesh->material->color.alpha * COLORSCALAR
                };

                if ((flags & (rxGEOMETRY_TEXTURED | rxGEOMETRY_TEXTURED2)) && mesh->material->texture)
                {
                    RwXboxRenderStateSetTexture(mesh->material->texture, 0);

                    ApplyEnvMapTextureMatrix(envMapData->texture, 1, envMapData->frame);

                    /*
                    * Pixel shader
                    */
                    RwXboxSetCurrentPixelShader(EnvMapPixelShader);
                }
                else
                {
                    ApplyEnvMapTextureMatrix(envMapData->texture, 0, envMapData->frame);

                    RwXboxSetCurrentPixelShader(EnvMapNoBaseTexturePixelShader);
                }

                D3DDevice_SetPixelShaderConstant(0, MaterialColor, 1);
                D3DDevice_SetPixelShaderConstant(1, ShinyFloats, 1);
            }
            else
            {
                if ((flags & (rxGEOMETRY_TEXTURED | rxGEOMETRY_TEXTURED2)) && mesh->material->texture)
                {
                    ...
                    /* Add to base texture */
                    RwXboxSetCachedTextureStageState(1, D3DTSS_COLOROP,   D3DTOP_MULTIPLYADD);
                    RwXboxSetCachedTextureStageState(1, D3DTSS_COLORARG0, D3DTA_CURRENT);
                    RwXboxSetCachedTextureStageState(1, D3DTSS_COLORARG1, D3DTA_TEXTURE);
                    RwXboxSetCachedTextureStageState(1, D3DTSS_COLORARG2, D3DTA_TFACTOR);

                    RwXboxSetCachedTextureStageState(1, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG2);
                ...
                else
                {
                    ...
                    /* Scale envmap by coef and add diffuse*/
                    RwXboxSetCachedTextureStageState(0, D3DTSS_COLOROP,   D3DTOP_MULTIPLYADD);
                    RwXboxSetCachedTextureStageState(0, D3DTSS_COLORARG0, D3DTA_DIFFUSE);
                    RwXboxSetCachedTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
                    RwXboxSetCachedTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TFACTOR);
                }
                ...
                RwXboxSetCachedRenderState(D3DRS_TEXTUREFACTOR, shinny);
        ...
        else
        {
            ...
            if ((flags & rxGEOMETRY_PRELIT) != 0 &&
                ...
                *((const RwUInt32 *)&(mesh->material->color)) != 0xffffffff)
            {
                RwUInt32 tFactor;
                const RwRGBA *color = &(mesh->material->color);

                tFactor = (color->alpha << 24) | (color->red << 16) | (color->green << 8) | color->blue;

                RwXboxSetCachedRenderState(D3DRS_TEXTUREFACTOR, tFactor );
        ...
        * Vertex shader
        */
        RwXboxSetCurrentVertexShader(mesh->vertexShader);
        ...
        */
        ...
        if (dualPass)
        {
            ...
            RwBool      fogEnabled;
            RwUInt32    fogColor;
            RwUInt32    srcBlend, destBlend;

            ApplyEnvMapTextureMatrix(envMapData->texture, 0, envMapData->frame);

            /* Scale envmap by coef */
            RwXboxSetCachedTextureStageState(0, D3DTSS_COLOROP,   D3DTOP_MODULATE);
            RwXboxSetCachedTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
            RwXboxSetCachedTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TEXTURE);

            RwXboxSetCachedTextureStageState(0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG2);
            RwXboxSetCachedTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TEXTURE);
            ...
            RwXboxGetCachedRenderState(D3DRS_LIGHTING, (void *)&lighting);
            RwXboxGetCachedRenderState(D3DRS_ZWRITEENABLE, (void *)&zWriteEnable);
            RwXboxGetCachedRenderState(D3DRS_FOGENABLE, (void *)&fogEnabled);

            RwXboxSetCachedRenderState(D3DRS_LIGHTING, FALSE);
            RwXboxSetCachedRenderState(D3DRS_ZWRITEENABLE, FALSE);

            if (fogEnabled)
            {
                RwXboxGetCachedRenderState(D3DRS_FOGCOLOR, (void *)&fogColor);
                RwXboxSetCachedRenderState(D3DRS_FOGCOLOR, 0);
            }

            RwXboxGetCachedRenderState( D3DRS_SRCBLEND, &srcBlend );
            RwXboxGetCachedRenderState( D3DRS_DESTBLEND, &destBlend );

            RwXboxSetCachedRenderState( D3DRS_SRCBLEND, D3DBLEND_SRCALPHA );
            RwXboxSetCachedRenderState( D3DRS_DESTBLEND, D3DBLEND_ONE );
            ...
            */
            ...
            if (fogEnabled)
            {
                RwXboxSetCachedRenderState(D3DRS_FOGCOLOR, fogColor);
            }

            RwXboxSetCachedRenderState(D3DRS_ZWRITEENABLE, zWriteEnable);
            RwXboxSetCachedRenderState(D3DRS_LIGHTING, lighting);

            RwXboxSetCachedRenderState( D3DRS_SRCBLEND, srcBlend );
            RwXboxSetCachedRenderState( D3DRS_DESTBLEND, destBlend );
        }
        else
        {
            if ((flags & rxGEOMETRY_PRELIT) &&
	...
	if(bUseSpecular && bLightingEnabled)
	{
		...
		//RwD3D9GetLight(CARFX_D3D8_SPEC_LIGHT_INDEX, &oldLight1);
	
		D3DLIGHT8 *pLight = &m_gSpecularLight;
		D3DDevice_SetLight(CARFX_D3D8_SPEC_LIGHT_INDEX,			pLight);
		D3DDevice_LightEnable(CARFX_D3D8_SPEC_LIGHT_INDEX,		TRUE);
		...
		D3DDevice_SetRenderState(D3DRS_LOCALVIEWER,				FALSE);				// simplified specular version
		D3DDevice_SetRenderState(D3DRS_SPECULARMATERIALSOURCE, D3DMCS_MATERIAL);	// Use the color from the current material. 
...
//		RwD3D9SetRenderState(D3DRS_SPECULARMATERIALSOURCE, D3DMCS_COLOR2);		// Use the specular vertex color. 
	...
	else
	{
		...
		materialSpecPower	= 0.0f;
			...
			// use the fixed function pipeline's lighting/materials:
			//_rxXbDefaultRenderFFPMeshSetUp(mesh);
			Custom_rxXbDefaultRenderFFPMeshSetUp(mesh, materialSpecular, materialSpecPower);
		...
		{
			D3DDevice_SetRenderState(D3DRS_SPECULARENABLE,		FALSE);
			D3DDevice_LightEnable(CARFX_D3D8_SPEC_LIGHT_INDEX,	FALSE);
		}

/////////////////////////////////////////////////////////////////////////////////////
//	_rxXbDefaultRenderFFPMesh(resEntryHeader, mesh, flags );
/////////////////////////////////////////////////////////////////////////////////////


        RwXboxSetCachedTextureStageState(0, D3DTSS_TEXTURETRANSFORMFLAGS,	D3DTTFF_DISABLE);
        RwXboxSetCachedTextureStageState(0, D3DTSS_TEXCOORDINDEX,			0);

        RwXboxSetCachedTextureStageState(1, D3DTSS_COLOROP,					D3DTOP_DISABLE);
        RwXboxSetCachedTextureStageState(1, D3DTSS_TEXTURETRANSFORMFLAGS,	D3DTTFF_DISABLE);
        RwXboxSetCachedTextureStageState(1, D3DTSS_TEXCOORDINDEX,			1);
...
//	dbgCheckMaterialRenderPassBits(matPassMask, pMaterial, pAtomic);
	...
	if(bUseSpecular && bLightingEnabled)
	{
		...
		RwD3D9SetLight(CARFX_D3D8_SPEC_LIGHT_INDEX,			pLight);
		RwD3D9EnableLight(CARFX_D3D8_SPEC_LIGHT_INDEX,		TRUE);
...
//
//
RxPipeline*	CCustomCarEnvMapPipeline::CreateCustomD3D8ObjPipe()
{
    ...
    if(pipe)
    {
        ...
        if(lpipe)
        {
            ...
            RxNodeDefinition *instanceNode = RxNodeDefinitionGetXboxAtomicAllInOne();
...
//				RxD3D9AllInOneSetReinstanceCallBack(allinone,	CustomPipeReinstanceCB);	// RxD3D9AllInOneGetReinstanceCallBack(allinone));
				RxXboxAllInOneSetRenderCallBack(	allinone,	CustomPipeRenderCB);		// RxD3D9AllInOneGetRenderCallBack(allinone));
...
{
	ObjPipeline = CreateCustomD3D8ObjPipe();
	if(!ObjPipeline)
	{
		return(FALSE);
	}
	...
	ZeroMemory(&m_gSpecularLight, sizeof(D3DLIGHT8));
